...
QUESTION_OUTCOME_SCHEMA_VERSION = "1.0"
...
class ResolutionEvidenceRef:
    ...
    def from_wire(cls, value: Mapping[str, Any]) -> "ResolutionEvidenceRef":
        return cls(
            evidence_family=str(value.get("evidence_family", "")),
...
class QuestionBoundOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        for field in ("prediction_id", "question_ref", "answer_kind", "outcome_metric_id", "resolver_policy_id", "resolver_implementation_ref"):
    ...
    def body(self) -> Dict[str, Any]:
        return {
            ...
            "timing": {
                ...
                "target_resolves_at_ns": self.target_resolves_at_ns,
    ...
    def from_wire(cls, value: Mapping[str, Any]) -> "QuestionBoundOutcome":
        ...
        item = cls(
            ...
            resolver_implementation_ref=str(question.get("resolver_implementation_ref", "")),
            status=str(value.get("status", "")),
            cutoff_at_ns=int(timing.get("cutoff_at_ns", -1)),
            target_resolves_at_ns=int(timing.get("target_resolves_at_ns", -1)),
            max_resolution_lag_ns=int(timing.get("max_resolution_lag_ns", -1)),
            decided_at_ns=int(timing.get("decided_at_ns", -1)),
            realized_answer=None if answer is None else dict(answer),
            resolution_evidence=tuple(ResolutionEvidenceRef.from_wire(item) for item in refs_raw if isinstance(item, Mapping)),
        )
        if len(item.resolution_evidence) != len(refs_raw):
