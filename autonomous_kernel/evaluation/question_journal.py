...
def _parse_entry(value: Mapping[str, Any]) -> Tuple[QuestionBoundOutcome, str]:
    ...
    if value.get("entry_hash") != expected:
        raise QuestionOutcomeJournalError("question outcome journal entry hash mismatch")
    try:
        outcome = QuestionBoundOutcome.from_wire(value.get("outcome", {}))
    except (QuestionOutcomeError, ValueError, TypeError) as exc:
        raise QuestionOutcomeJournalError("journal contains invalid question outcome: %s" % exc) from exc
...
def _prediction_lineage_error(root: Path, outcome: QuestionBoundOutcome) -> Optional[str]:
    ...
    if prediction_errors:
        return "question prediction journal invalid: " + "; ".join(prediction_errors)
    ...
    if len(matches) != 1:
        return "outcome must cite exactly one journaled question prediction"
    entry = matches[0]
    try:
        ...
        return "cited question prediction is invalid: %s" % exc
    checks = (
...
class QuestionOutcomeJournal:
    """Append-only final market outcomes for question-bound predictions."""

    def __init__(self, root: Path) -> None:
        self.root = root.resolve()
        self.path = self.root / "memory/question_outcomes.jsonl"
        self.state_path = self.root / "state/question_outcome_journal.json"
    ...
    def entries(self) -> Tuple[Mapping[str, Any], ...]:
        ...
        for line_number, line in enumerate(self.path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            try:
                ...
                raise QuestionOutcomeJournalError("question outcome journal line %d is invalid JSON" % line_number) from exc
            if not isinstance(value, dict):
                raise QuestionOutcomeJournalError("question outcome journal line %d must be an object" % line_number)
            output.append(value)
        return tuple(output)
    ...
    def append(self, outcome: QuestionBoundOutcome) -> Mapping[str, Any]:
        ...
        if lineage_error:
            raise QuestionOutcomeJournalError("outcome prediction lineage invalid: " + lineage_error)
        with writer_lock(self.root):
            records = self.entries()
            errors = _validate_records(records)
            if errors:
                raise QuestionOutcomeJournalError("existing question outcome journal is invalid: " + "; ".join(errors))
            for record in records:
                existing, _ = _parse_entry(record)
                if existing.prediction_id != outcome.prediction_id:
                    continue
                if existing.to_wire() != outcome.to_wire():
                    raise QuestionOutcomeJournalError("prediction already has a different final question outcome")
    ...
    def _write_state(self, records: Tuple[Mapping[str, Any], ...]) -> None:
        state = {"schema_version": 1, "question_outcome_contract_version": "1.0", "authority": "append-only market outcomes; evaluation scores are separate", "entry_count": len(records), "last_sequence": None if not records else int(records[-1]["sequence"]), "last_entry_hash": None if not records else str(records[-1]["entry_hash"])}
        _atomic_json(self.state_path, state)

    def rebuild_state(self) -> Mapping[str, Any]:
        records = self.entries(); errors = _validate_records(records)
        if errors:
            raise QuestionOutcomeJournalError("question outcome journal cannot rebuild state: " + "; ".join(errors))
        self._write_state(records)
...
def _validate_records(records: Tuple[Mapping[str, Any], ...]) -> List[str]:
    ...
    for index, value in enumerate(records):
        ...
        except QuestionOutcomeJournalError as exc:
            errors.append("sequence %d: %s" % (index, exc)); continue
        ...
        if outcome.prediction_id in seen:
...
def validate_question_outcome_journal(root: Path) -> List[str]:
    ...
    if not errors:
        for index, record in enumerate(records):
            ...
            except QuestionOutcomeJournalError:
                continue
    ...
    try:
        ...
        errors.append("question outcome journal state unreadable: %s" % exc); return errors
    if state.get("schema_version") != 1 or state.get("question_outcome_contract_version") != "1.0":
        errors.append("question outcome journal state schema invalid")
    if state.get("entry_count") != len(records):
        errors.append("question outcome journal state count mismatch")
    ...
    if state.get("last_sequence") != expected_sequence:
        errors.append("question outcome journal state last_sequence mismatch")
    if state.get("last_entry_hash") != expected_hash:
        errors.append("question outcome journal state last_entry_hash mismatch")
    return errors
