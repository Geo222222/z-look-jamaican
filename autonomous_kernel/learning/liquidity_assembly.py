"""Durable same-question Liquidity assembly from earned journal competence.
...
from .liquidity_loop import HORIZON_NS, LIQUIDITY_QUESTION_REF


class LiquidityAssemblyError(RuntimeError):
    pass


EXECUTABLE_LIQUIDITY_MODEL_IDS = ("LIQUIDITY-NULL-PRIOR", "SPREAD-DEPTH-PRESSURE", "BOOK-DEPLETION-STRESS")
        ...
        for key, item in value.items():
            ...
            for forbidden in FORBIDDEN_FIELDS:
                if forbidden in lowered:
                    raise LiquidityAssemblyError("assembly contains forbidden field %s" % key)
...
def _liquidity_contracts() -> Tuple[Mapping[str, Any], ...]:
    contracts = []
    for contract in implemented_baseline_expert_contracts():
        if LIQUIDITY_QUESTION_REF not in contract["question_refs"]:
            continue
        model_ids = tuple(str(ref).split("@", 1)[0] for ref in contract.get("model_refs") or ())
        if any(model_id in EXECUTABLE_LIQUIDITY_MODEL_IDS for model_id in model_ids):
            contracts.append(contract)
    if len(contracts) != 3:
        raise LiquidityAssemblyError("expected three executable Liquidity experts")
...
def _predictions(root: Path) -> Tuple[Tuple[QuestionBoundPrediction, int], ...]:
    ...
    if errors:
        raise LiquidityAssemblyError("question prediction journal invalid: " + "; ".join(errors))
    output = []
...
def _outcomes(root: Path) -> Dict[str, QuestionBoundOutcome]:
    ...
    if errors:
        raise LiquidityAssemblyError("question outcome journal invalid: " + "; ".join(errors))
    ...
    for entry in QuestionOutcomeJournal(root).entries():
        ...
        if outcome.prediction_id in output:
            raise LiquidityAssemblyError("prediction has duplicate question outcomes")
...
def _covers_executable(items: Sequence[Mapping[str, Any]]) -> bool:
    ...
    return models.issuperset(EXECUTABLE_LIQUIDITY_MODEL_IDS)
    ...
    *,
    ...
    if cutoff_at_ns is not None:
        ...
        if not _covers_executable(selected):
            raise LiquidityAssemblyError("requested Liquidity cutoff lacks the three executable claims")
        return int(cutoff_at_ns)
    eligible = [cutoff for cutoff, items in grouped.items() if _covers_executable(items)]
    if not eligible:
        raise LiquidityAssemblyError("no Liquidity cutoff has three executable journaled claims")
    return max(eligible)
...
def _unique_executable(selected: Sequence[Mapping[str, Any]]) -> List[Mapping[str, Any]]:
    ...
    for row in selected:
        ...
        if model_id not in EXECUTABLE_LIQUIDITY_MODEL_IDS:
            continue
        ...
        if new_hash < prior_hash:
            by_model[model_id] = row
    if set(by_model) != set(EXECUTABLE_LIQUIDITY_MODEL_IDS):
        raise LiquidityAssemblyError("Liquidity assembly requires the three distinct executable experts")
    output = list(by_model.values())
    output.sort(key=lambda item: (str(item["claim"]["expert_ref"]), str(item["claim"]["integrity"]["content_hash"])))
    return output


def assemble_liquidity_question(
    root: Path,
    *,
    known_at_ns: int,
    cutoff_at_ns: Optional[int] = None,
) -> Mapping[str, Any]:
    """Assemble executable Liquidity claims from durable journals and competence."""
    root = Path(root).resolve()
    known_at = int(known_at_ns)
    if known_at < 0:
        raise LiquidityAssemblyError("known_at_ns must be non-negative")
    runtime = IntelligenceRuntime(root)
    chain_errors = validate_event_chain(runtime.events())
    if chain_errors:
        raise LiquidityAssemblyError("expert intelligence journal invalid: " + "; ".join(chain_errors))
    state = runtime.state()
    ...
    if isinstance(persisted, Mapping) and persisted.get("known_at_ns") is not None and int(persisted["known_at_ns"]) > known_at:
        raise LiquidityAssemblyError("future competence cannot enter earlier assembly")
    contracts = _liquidity_contracts()
    contract_by_model = {}
    ...
    for prediction, journaled_at in _predictions(root):
        if journaled_at > known_at:
            continue
        if prediction.question_ref != LIQUIDITY_QUESTION_REF:
            continue
        ...
        if not isinstance(recorded_claim, Mapping):
            raise LiquidityAssemblyError("Liquidity claim is not in expert runtime: %s" % claim_hash)
        if recorded_claim.get("integrity", {}).get("content_hash") != claim_hash:
            raise LiquidityAssemblyError("recorded Liquidity claim hash mismatch")
    ...
    for row in selected:
        if str(row["prediction"].subject_id) != SUBJECT_ID:
            raise LiquidityAssemblyError("wrong subject cannot contribute")
        if row["prediction"].question_ref != LIQUIDITY_QUESTION_REF:
            raise LiquidityAssemblyError("wrong-question claims cannot contribute")
        if not row["scored"]:
            raise LiquidityAssemblyError("unresolved or unscored claims cannot contribute")
        outcome = row["outcome"]
        if outcome is None or outcome.status != "RESOLVED":
            raise LiquidityAssemblyError("unresolved or unscored claims cannot contribute")
        if int(outcome.decided_at_ns) > known_at:
            raise LiquidityAssemblyError("future outcomes cannot enter earlier assembly")
    ...
    envelope = {
        "schema_version": "1.0",
        "assembly_id": "QASM-%s" % canonical_hash({"assembly": assembly, "cutoff": cutoff, "known_at": known_at, "family": "LIQUIDITY"})[:32],
...
def assemble_and_record_liquidity_question(
    root: Path,
    *,
    known_at_ns: int,
    cutoff_at_ns: Optional[int] = None,
) -> Mapping[str, Any]:
    envelope = assemble_liquidity_question(root, known_at_ns=known_at_ns, cutoff_at_ns=cutoff_at_ns)
    runtime = IntelligenceRuntime(root)
...
def liquidity_assembly_projection(root: Path) -> Mapping[str, Any]:
    root = Path(root).resolve()
    runtime = IntelligenceRuntime(root)
    assemblies = [
        item
        for item in (runtime.state().get("assemblies") or [])
        if isinstance(item, Mapping) and item.get("question_ref") == LIQUIDITY_QUESTION_REF
    ]
