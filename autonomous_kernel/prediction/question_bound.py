...
def _validate_answer(answer_kind: AnswerKind, answer: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if answer_kind is AnswerKind.BINARY:
        ...
        low = answer.get("interval_low")
        high = answer.get("interval_high")
        ...
        if low is not None:
            ...
            normalized["interval_low"] = low_text
            normalized["interval_high"] = high_text
...
class QuestionBoundPrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode not in QUESTION_PREDICTION_MODES:
            raise QuestionPredictionError("prediction mode is invalid")
        if self.evidence_class not in QUESTION_PREDICTION_EVIDENCE_CLASSES:
            raise QuestionPredictionError("prediction evidence_class is invalid")
        ...
        if self.mode == "PROSPECTIVE_SHADOW":
            ...
            if self.created_at_ns > self.resolves_at_ns:
                raise QuestionPredictionError("prospective prediction cannot be created after resolution horizon")
        ...
        _validate_answer(AnswerKind(self.answer_kind), self.answer)
    ...
    def from_wire(cls, value: Mapping[str, Any]) -> "QuestionBoundPrediction":
        ...
        item = cls(
            ...
            question_definition_hash=str(question.get("definition_hash", "")),
            question_registry_id=str(registry.get("registry_id", "")),
            question_registry_version=str(registry.get("version", "")),
            question_registry_hash=str(registry.get("content_hash", "")),
            question_family=str(question.get("family", "")),
            question_scope=str(question.get("scope", "")),
            answer_kind=str(question.get("answer_kind", "")),
            cutoff_at_ns=int(timing.get("cutoff_at_ns", -1)),
            ...
            outcome_metric_id=str(question.get("outcome_metric_id", "")),
            resolver_policy_id=str(question.get("resolver_policy_id", "")),
            max_resolution_lag_ns=int(question.get("max_resolution_lag_ns", -1)),
            answer=dict(answer),
            model_refs=tuple(str(item) for item in value.get("model_refs", [])),
            artifact_refs=tuple(PredictionArtifactRef.from_wire(item) for item in refs_raw if isinstance(item, Mapping)),
        )
        if len(item.artifact_refs) != len(refs_raw):
...
def build_question_bound_prediction(
    *,
    ...
    if mode == "PROSPECTIVE_SHADOW" and entry.lifecycle_state not in {"RESOLVER_READY", "QUALIFIED"}:
    ...
    if registry.known_at_ns > cutoff_at_ns or entry.effective_at_ns > cutoff_at_ns:
        raise QuestionPredictionError("question registry was not knowable/effective at prediction cutoff")

    refs = tuple(artifact_refs)
    artifact_types = {ref.artifact_type for ref in refs}
    feature_families = {family for ref in refs for family in ref.feature_families}
    ...
    if not set(question.required_feature_families).issubset(feature_families):
    ...
    if feature_families.intersection(set(question.forbidden_feature_families)):
        raise QuestionPredictionError("forbidden feature family is present")
    if not feature_families.issubset(set(question.allowed_feature_families)):
    ...
    normalized_answer = _validate_answer(question.outcome.answer_kind, answer)
    model_tuple = _unique_strings(model_refs, "model_refs")
    material = {
        ...
        "model_refs": list(model_tuple),
        "artifact_refs": [ref.to_wire() for ref in sorted(refs, key=lambda item: (item.artifact_type, item.artifact_id))],
    }
    prediction_id = "QPRED-%s" % hashlib.sha256(str(canonical_hash(material)).encode("utf-8")).hexdigest()[:32]
    return QuestionBoundPrediction(
        ...
        question_registry_version=registry.version,
        question_registry_hash=registry.content_hash(),
        question_family=question.family.value,
        question_scope=question.scope.value,
        answer_kind=question.outcome.answer_kind.value,
        cutoff_at_ns=int(cutoff_at_ns),
        created_at_ns=int(created_at_ns),
        horizon_ns=question.horizon_ns,
        resolves_at_ns=int(cutoff_at_ns) + question.horizon_ns,
        outcome_metric_id=question.outcome.metric_id,
        resolver_policy_id=question.outcome.resolver_policy_id,
        max_resolution_lag_ns=question.outcome.max_resolution_lag_ns,
        answer=normalized_answer,
        model_refs=model_tuple,
        artifact_refs=refs,
    )
