...
from autonomous_kernel.learning.magnitude_loop import _cutoffs, process_canonical_magnitude_batches
from autonomous_kernel.synthesis.service import market_synthesis_projection


REPO = Path(__file__).resolve().parents[1]
REAL_MAGNITUDE_OPT_IN_ENV = "ZLOOK_RUN_REAL_MAGNITUDE_TEST"
...
def _opt_in_requested() -> bool:
    return os.environ.get(REAL_MAGNITUDE_OPT_IN_ENV, "").strip() == "1"
...
def require_opt_in_local_coinbase_captures(root: Path) -> list:
    if not _opt_in_requested():
        raise unittest.SkipTest(
            "real Coinbase Magnitude proof is opt-in; set %s=1 with local CAN-COINBASE-BTC-USD-OBS-* captures"
            % REAL_MAGNITUDE_OPT_IN_ENV
        )
...
class RealCoinbaseMagnitudeGateTests(unittest.TestCase):
    def test_skips_without_opt_in_and_does_not_mutate_checkout(self):
        before = _checkout_guard(REPO)
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop(REAL_MAGNITUDE_OPT_IN_ENV, None)
    ...
    def test_opt_in_without_local_captures_fails_clearly(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {REAL_MAGNITUDE_OPT_IN_ENV: "1"}):
                with self.assertRaisesRegex(AssertionError, r"%s=1 but no .*captures are present" % REAL_MAGNITUDE_OPT_IN_ENV):
                    require_opt_in_local_coinbase_captures(empty)
        self.assertEqual(before, _checkout_guard(REPO))


class RealCoinbaseMagnitudeLoopTests(unittest.TestCase):
    def test_public_coinbase_batches_complete_magnitude_school_loop(self):
        before = _checkout_guard(REPO)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            if not usable:
                raise AssertionError("%s=1 but copied captures are too short for a frozen 30s Magnitude cutoff" % REAL_MAGNITUDE_OPT_IN_ENV)
            direction = process_canonical_direction_batches(root, tuple(usable), sync=True)
            result = process_canonical_magnitude_batches(root, tuple(usable), sync=True)
            self.assertGreaterEqual(result["counts"]["predicted"], 3)
            self.assertGreater(result["counts"]["resolved"] + result["counts"]["unresolvable"], 0)
            self.assertEqual(result["question_ref"], "ECONOMIC_ROOT_MAGNITUDE_30S@1.0.0")
            self.assertEqual(result["horizon_ns"], 30_000_000_000)
            self.assertFalse(result["authority"]["capital_allocation"])
            assembly = (result.get("sync") or {}).get("magnitude_assembly") or {}
        ...
        self.assertEqual(
            ...
            "real Coinbase Magnitude test must not write runtime/, memory/, or state/ in the repository checkout",
        )
