"""Bounded Volatility 60s predict → resolve → score → competence loop.

Reuses QuestionPredictionJournal, QuestionOutcomeJournal, resolve_fixed_grid_question,
the Market-Wide Experience Bridge, and sync_expert_learning. Does not train models
or change resolver truth. Research-only while Z9/experience is not QUALIFIED.
"""
...
from ..evaluation.question_path_resolvers import resolve_fixed_grid_question
from ..evaluation.question_resolvers import QuestionOutcomePendingError
from ..experience.bridge import MarketWideExperienceBridgeError, materialize_market_wide_experience
...
from ..models.volatility_baselines import VOLATILITY_HORIZON_NS, population_stdev_bps, volatility_baseline_model_set
...
VOLATILITY_QUESTION_REF = "ECONOMIC_ROOT_VOLATILITY_60S@1.0.0"
HORIZON_NS = VOLATILITY_HORIZON_NS
LAG_NS = 5_000_000_000
GRID_NS = 5_000_000_000
SHORT_LOOKBACK_NS = 60_000_000_000


class VolatilityLoopError(RuntimeError):
    pass


def volatility_question():
    matches = [item for item in question_catalog_v1() if item.question_ref == VOLATILITY_QUESTION_REF]
    if len(matches) != 1:
        raise VolatilityLoopError("frozen Volatility question is missing")
    return matches[0]


def volatility_model_set() -> Tuple[Any, ...]:
    return tuple(volatility_baseline_model_set())


def _cutoffs(start_ns: int, end_ns: int) -> Tuple[int, ...]:
    first = start_ns + SHORT_LOOKBACK_NS
...
def volatility_baseline_is_eligible(frame: RepresentationFrame) -> bool:
    return (
        frame.instrument.canonical_id == INSTRUMENT_ID
...
def materialize_short_cutoff_frame(root: Path, batch_id: str, cutoff_at_ns: int) -> RepresentationFrame:
    ...
    if frame.known_at_ns > cutoff_at_ns or frame.cutoff_at_ns > cutoff_at_ns:
        raise VolatilityLoopError("SHORT cutoff frame leaked post-cutoff evidence")
    if frame.window_start_ns < cutoff_at_ns - SHORT_LOOKBACK_NS:
        raise VolatilityLoopError("SHORT cutoff frame lookback exceeds Volatility SHORT timescale")
    return frame


def _short_baseline_frame(root: Path, experience) -> RepresentationFrame:
    ...
    if len(views) != 1 or len(views[0].source_frames) != 1:
        raise VolatilityLoopError("Volatility experience must bind exactly one SHORT source frame")
...
def trailing_returns_bps(root: Path, batch_id: str, cutoff_at_ns: int) -> Tuple[Decimal, ...]:
    prices: List[Decimal] = []
    start = int(cutoff_at_ns) - SHORT_LOOKBACK_NS
    target = start
    while target <= int(cutoff_at_ns):
        try:
            artifact = materialize_instrument_state(
                root,
                batch_ids=(batch_id,),
                instrument_id=INSTRUMENT_ID,
                cutoff_at_ns=target,
                min_known_at_ns=max(0, target - GRID_NS),
                quality_statuses=("VALID",),
            )
            frame = _load_frame(root, artifact)
        except (RepresentationMaterializationError, VolatilityLoopError, ValueError):
            target += GRID_NS
            continue
        if frame.known_at_ns > cutoff_at_ns or frame.cutoff_at_ns > cutoff_at_ns:
            target += GRID_NS
            continue
        price = _midpoint(frame)
        if price is not None and price > 0:
            prices.append(price)
        target += GRID_NS
    if len(prices) < 2:
        return ()
    return tuple((prices[index] / prices[index - 1] - Decimal("1")) * Decimal("10000") for index in range(1, len(prices)))


def materialize_volatility_forward_frames(root: Path, batch_id: str, cutoff_at_ns: int, resolves_at_ns: int) -> Tuple[RepresentationFrame, ...]:
    frames: List[RepresentationFrame] = []
    grid = tuple(range(int(cutoff_at_ns) + GRID_NS, int(resolves_at_ns) + 1, GRID_NS))
    for index, target in enumerate(grid):
        policy_upper = target + LAG_NS
        ...
        try:
            artifact = materialize_instrument_state(
                ...
                batch_ids=(batch_id,),
                instrument_id=INSTRUMENT_ID,
                cutoff_at_ns=slot_upper,
                min_known_at_ns=target,
                quality_statuses=("VALID",),
            )
            frame = _load_frame(root, artifact)
        except (RepresentationMaterializationError, ValueError):
            continue
        if frame.known_at_ns < target or frame.known_at_ns > slot_upper:
            continue
        if frame.known_at_ns <= cutoff_at_ns:
            continue
        frames.append(frame)
    return tuple(frames)
...
def _experience_for_cutoff(root: Path, micro: RepresentationFrame, short: RepresentationFrame) -> Tuple[Any, Any, MarketWideExperienceState]:
    cutoff = int(short.cutoff_at_ns)
    _direction_experience, context = _direction_experience_for_cutoff(root, micro)
    del _direction_experience
    try:
        ...
        raise VolatilityLoopError("MARKET_WIDE_EXPERIENCE unavailable at cutoff: %s" % exc) from exc
    if market_wide.known_at_ns > cutoff or market_wide.cutoff_at_ns > cutoff:
        raise VolatilityLoopError("market-wide experience leaked past prediction cutoff")
    if market_wide.status == "UNAVAILABLE":
        raise VolatilityLoopError("MARKET_WIDE_EXPERIENCE is UNAVAILABLE")
    # MICRO is retained for Z9/context lineage; the frozen exam still requires SHORT.
...
def _continuous_answer(value: Decimal) -> Dict[str, Any]:
    if value < 0:
        raise VolatilityLoopError("predicted volatility must be non-negative")
...
def record_volatility_predictions(
    root: Path,
    *,
    batch_id: str,
    ...
    trailing: Sequence[Decimal] = (),
) -> List[Mapping[str, Any]]:
    if not volatility_baseline_is_eligible(frame):
        return []
    if market_wide is None:
        return []
    registry = frozen_direction_registry()
    question = volatility_question()
    # PredictionArtifactRef admits only QUALIFIED/DEGRADED. Map UNAVAILABLE experience
    # (common when Z9 context is UNAVAILABLE) onto DEGRADED research replay, matching Direction.
    ...
    market_wide_artifact_status = "DEGRADED" if market_wide.status != "QUALIFIED" else "QUALIFIED"
    ...
    market_wide_ref = PredictionArtifactRef(
        ...
        status=market_wide_artifact_status,
        timescales=(ExperienceTimescale.SHORT,),
    ...
    for entry in journal.entries():
        ...
        if question_ref != VOLATILITY_QUESTION_REF:
            continue
    ...
    for model in volatility_model_set():
        if (int(frame.cutoff_at_ns), model.definition.model_ref) in existing:
            match = next(
                ...
                and ((entry.get("prediction") or {}).get("question") or {}).get("question_ref") == VOLATILITY_QUESTION_REF
            )
            recorded.append(_prediction_summary(match))
            continue
        expected, decomposition = model.forecast_volatility(frame, market_wide, trailing_returns_bps=trailing)
        del decomposition
...
def resolve_volatility_prediction(
    root: Path,
    *,
    batch_id: str,
    prediction_id: str,
    ...
    if prediction is None:
        raise VolatilityLoopError("prediction is not in the question journal")
    if now_at_ns < prediction.resolves_at_ns:
        return {"status": "PENDING", "prediction_id": prediction_id, "error": "horizon has not matured"}
    lineage_baseline = _short_baseline_frame(root, experience)
    if (
        baseline_frame.frame_id != lineage_baseline.frame_id
        or baseline_frame.content_hash() != lineage_baseline.content_hash()
    ):
        raise VolatilityLoopError("supplied baseline frame diverges from experience lineage")
    if lineage_baseline.status != "QUALIFIED":
        raise VolatilityLoopError("Volatility resolver requires a QUALIFIED SHORT baseline instrument state")
    forward = materialize_volatility_forward_frames(root, batch_id, prediction.cutoff_at_ns, prediction.resolves_at_ns)
    try:
    ...
    if outcome.status == "RESOLVED":
        realized = Decimal(str((outcome.realized_answer or {}).get("value") or "0"))
        if realized < 0:
            raise VolatilityLoopError("realized volatility must be non-negative")
    ...
    return {
        ...
        "forward_frame_count": len(forward),
    }


def _sync_volatility(root: Path, known_at_ns: int) -> Mapping[str, Any]:
    ...
    from .volatility_assembly import VolatilityAssemblyError, assemble_and_record_volatility_question

    try:
        sync_result["volatility_assembly"] = assemble_and_record_volatility_question(root, known_at_ns=int(known_at_ns))
    except VolatilityAssemblyError as exc:
        sync_result["volatility_assembly"] = {"status": "BLOCKED", "error": str(exc)}
        return sync_result
...
def process_canonical_volatility_batch(root: Path, batch_id: str, *, sync: bool = True) -> Mapping[str, Any]:
    root = Path(root).resolve()
    start_ns, end_ns = _observation_bounds(root, batch_id)
    cutoffs = _cutoffs(start_ns, end_ns)
    if not cutoffs:
        raise VolatilityLoopError("canonical batch is too short for a Volatility 60s independent cutoff")
    predictions: List[Mapping[str, Any]] = []
    experiences: Dict[int, Tuple[Any, Any, MarketWideExperienceState]] = {}
    skipped_invalid_cutoffs: List[Mapping[str, Any]] = []
    for cutoff in cutoffs:
        try:
            micro = materialize_cutoff_frame(root, batch_id, cutoff)
            ...
            continue
        if not volatility_baseline_is_eligible(short):
            skipped_invalid_cutoffs.append({"cutoff_at_ns": int(cutoff), "reason": "UNQUALIFIED_OR_ILLEGAL_BASELINE"})
            continue
        try:
            experience, context, market_wide = _experience_for_cutoff(root, micro, short)
        except VolatilityLoopError as exc:
            skipped_invalid_cutoffs.append({"cutoff_at_ns": int(cutoff), "reason": str(exc)})
            continue
        trailing = trailing_returns_bps(root, batch_id, cutoff)
        experiences[cutoff] = (experience, context, market_wide)
        predictions.extend(
            record_volatility_predictions(
                root,
                batch_id=batch_id,
                frame=short,
    ...
    if not predictions:
        raise VolatilityLoopError("canonical batch has no legal Volatility 60s cutoff that can be examined")
    ...
    for item in predictions:
        ...
        outcomes.append(
            resolve_volatility_prediction(
                ...
                baseline_frame=_short_baseline_frame(root, experience),
                experience=experience,
                now_at_ns=end_ns,
            )
        )
    ...
    if sync:
        sync_result = _sync_volatility(root, end_ns)
    counts = {
    ...
    latest_bundle = experiences[latest_cutoff] if latest_cutoff is not None else None
    latest_context = None if latest_bundle is None else latest_bundle[1]
    latest_market_wide = None if latest_bundle is None else latest_bundle[2]
    latest_experience = None if latest_bundle is None else latest_bundle[0]
    ...
    return {
        "status": "OK",
        "batch_id": batch_id,
        "question_ref": VOLATILITY_QUESTION_REF,
        ...
        "counts": counts,
        "sync": sync_result,
        "short_experience_status": None if latest_experience is None else latest_experience.status,
        "market_wide_experience_status": None if latest_market_wide is None else latest_market_wide.status,
        "z9_status": None if latest_context is None else getattr(latest_context, "status", None),
...
def process_canonical_volatility_batches(root: Path, batch_ids: Sequence[str], *, sync: bool = True) -> Mapping[str, Any]:
    ...
    for batch_id in batch_ids:
        result = process_canonical_volatility_batch(root, str(batch_id), sync=False)
        ...
        combined["short_experience_status"] = result.get("short_experience_status")
        combined["market_wide_experience_status"] = result.get("market_wide_experience_status")
        combined["z9_status"] = result.get("z9_status")
    ...
    if sync and last_end:
        combined["sync"] = _sync_volatility(root, last_end)
    return combined
