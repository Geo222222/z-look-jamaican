...
def _frame(cutoff=10 * SECOND):
    return RepresentationFrame(
        ...
        parameters={},
        state={"venue_states": {}, "aggregate": {"qualified_book_venue_count": 1, "venue_count": 1}},
...
def _context(frame, cutoff=10 * SECOND):
    return MarketContextFrame(
        ...
        parameters={},
        state={
            "members": {frame.instrument.canonical_id: {"frame_id": frame.frame_id, "frame_content_hash": frame.content_hash()}},
...
class PerceptionGraphTests(unittest.TestCase):
    ...
    def test_graph_forbids_strategy_and_opportunity_authority(self):
        with self.assertRaisesRegex(PerceptionGraphError, "outside ZLJ perception authority"):
            build_graph_node(
                node_id="STRATEGY-1", node_type="STRATEGY_APPLICABILITY", truth_class="APPLICABILITY_ASSESSMENT",
                subject_id="CRYPTO.SPOT.BTC-USD", cutoff_at_ns=10, known_at_ns=10,
                source_refs=("source:a",), input_refs=(), method={"name": "old", "version": "1"},
                quality={"status": "VALID"}, payload={},
            )

    def test_graph_forbids_same_layer_and_upward_dependencies(self):
        base = build_graph_node(
            node_id="DERIVED-A", node_type="DETERMINISTIC_DERIVATION", truth_class="DETERMINISTIC_CALCULATION",
            subject_id="CRYPTO.SPOT.BTC-USD", cutoff_at_ns=10, known_at_ns=9, source_refs=("source:a",),
            input_refs=(), method={"name": "calc", "version": "1"}, quality={"status": "VALID"}, payload={},
        )
        peer = build_graph_node(
            node_id="DERIVED-B", node_type="DETERMINISTIC_DERIVATION", truth_class="DETERMINISTIC_CALCULATION",
            subject_id="CRYPTO.SPOT.BTC-USD", cutoff_at_ns=10, known_at_ns=9, source_refs=("source:b",),
            input_refs=(GraphRef(node_id=base["node_id"], relationship="PEER"),), method={"name": "calc", "version": "1"},
            quality={"status": "VALID"}, payload={},
        )
    ...
    def test_graph_forbids_point_in_time_lookahead(self):
        parent = build_graph_node(
            node_id="REP-FUTURE", node_type="REPRESENTATION", truth_class="NORMALIZED_MEASUREMENT",
            subject_id="CRYPTO.SPOT.BTC-USD", cutoff_at_ns=20, known_at_ns=19, source_refs=("observation:future",),
            input_refs=(), method={"name": "z2", "version": "1"}, quality={"status": "QUALIFIED"}, payload={},
        )
        child = build_graph_node(
            node_id="CTX-PAST", node_type="MARKET_CONTEXT", truth_class="DERIVED_CONTEXT", subject_id="MARKET.WIDE",
            cutoff_at_ns=10, known_at_ns=10, source_refs=("representation:future",),
            input_refs=(GraphRef(node_id=parent["node_id"], relationship="MEMBER"),),
            method={"name": "z9", "version": "1"}, quality={"status": "QUALIFIED"}, payload={},
        )
