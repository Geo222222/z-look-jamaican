...
class CompetenceTests(unittest.TestCase):
    def test_metrics_are_segmented_and_calibrated_from_resolved_evidence_only(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("20.00", profile.metrics["mean_absolute_error_bps"])
            self.assertEqual("20.00", profile.metrics["root_mean_squared_error_bps"])
            self.assertEqual("20.00", profile.metrics["mean_forecast_bias_bps"])
            self.assertEqual("1", profile.metrics["directional_accuracy"])
            self.assertEqual("0.04", profile.metrics["brier_score"])
            self.assertEqual("0.5", profile.metrics["mean_probability_positive"])
            self.assertEqual("0.5", profile.metrics["actual_positive_rate"])
            self.assertEqual("0.0", profile.metrics["calibration_gap"])
            self.assertEqual("1", profile.metrics["interval_coverage"])
            self.assertEqual("0.03846153846153846153846153846", profile.metrics["sample_strength"])
