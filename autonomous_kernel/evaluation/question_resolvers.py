...
def _bound_market_experience(prediction: QuestionBoundPrediction, experience: MarketExperienceFrame) -> None:
    ...
    if prediction.mode == "PROSPECTIVE_SHADOW" and experience.status != "QUALIFIED":
        raise QuestionResolverError("prospective midpoint resolution requires qualified baseline experience")


def _baseline_spot_frame(
    prediction: QuestionBoundPrediction,
    experience: MarketExperienceFrame,
    frames: Sequence[RepresentationFrame],
) -> RepresentationFrame:
    micro_views = [view for view in experience.views if view.timescale is ExperienceTimescale.MICRO]
    if len(micro_views) != 1:
        raise QuestionResolverError("midpoint resolver requires exactly one MICRO experience view")
    source_refs = [
        ref
        for ref in micro_views[0].source_frames
        if ref.representation_type == "INSTRUMENT_STATE"
        and ref.market_type == "SPOT"
        and ref.status == "QUALIFIED"
        and ref.cutoff_at_ns == prediction.cutoff_at_ns
    ]
    instrument_ids = sorted({ref.instrument_id for ref in source_refs})
    if len(instrument_ids) != 1:
        raise QuestionResolverError(
            "midpoint resolver requires one unambiguous qualified spot instrument at cutoff"
        )
...
def _resolved_value(prediction: QuestionBoundPrediction, baseline_price: Decimal, realized_price: Decimal):
    ...
    if prediction.question_ref.startswith("ECONOMIC_ROOT_MAGNITUDE_30S@"):
        realized_return = (realized_price / baseline_price - Decimal("1")) * Decimal("10000")
        return {"value": format(realized_return, "f")}
    raise QuestionResolverError("unsupported midpoint question")


def resolve_midpoint_question(
    root: Path,
    prediction_id: str,
    *,
    ...
    """Resolve direction/magnitude without endpoint or instrument cherry-picking.

    The baseline spot instrument is frozen from the prediction-bound MICRO
    Market Experience. If more than one qualified spot expression is eligible,
    this resolver refuses to choose among them. Forward resolution may use only
    that same canonical instrument and the first independently knowable
    qualified frame inside the Question Definition's declared lag window.
    """
    root = root.resolve()
    ...
    if selected is not None:
        ...
        return QuestionBoundOutcome(
            ...
            realized_answer=_resolved_value(prediction, baseline_price, realized),
            resolution_evidence=(
                ResolutionEvidenceRef(
