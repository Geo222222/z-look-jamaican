"""Durable same-question Volatility assembly from earned journal competence.

Reuses assemble_expert_claims. Research/replay only. Does not publish internal
intelligence or qualify a Benjamin handoff.
"""
...
from .direction_assembly import _source_evidence_groups, _source_evidence_refs
from .direction_loop import FORBIDDEN_FIELDS, SUBJECT_ID
from .volatility_loop import HORIZON_NS, VOLATILITY_QUESTION_REF


class VolatilityAssemblyError(RuntimeError):
    pass


EXECUTABLE_VOLATILITY_MODEL_IDS = (
        ...
        for key, item in value.items():
            ...
            for forbidden in FORBIDDEN_FIELDS:
                if forbidden in lowered:
                    raise VolatilityAssemblyError("assembly contains forbidden field %s" % key)
...
def _volatility_contracts() -> Tuple[Mapping[str, Any], ...]:
    contracts = []
    for contract in implemented_baseline_expert_contracts():
        if VOLATILITY_QUESTION_REF not in contract["question_refs"]:
            continue
        model_ids = tuple(str(ref).split("@", 1)[0] for ref in contract.get("model_refs") or ())
        if any(model_id in EXECUTABLE_VOLATILITY_MODEL_IDS for model_id in model_ids):
            contracts.append(contract)
    if len(contracts) != 3:
        raise VolatilityAssemblyError("expected three executable Volatility experts")
...
def _predictions(root: Path) -> Tuple[Tuple[QuestionBoundPrediction, int], ...]:
    ...
    if errors:
        raise VolatilityAssemblyError("question prediction journal invalid: " + "; ".join(errors))
    output = []
...
def _outcomes(root: Path) -> Dict[str, QuestionBoundOutcome]:
    ...
    if errors:
        raise VolatilityAssemblyError("question outcome journal invalid: " + "; ".join(errors))
    ...
    for entry in QuestionOutcomeJournal(root).entries():
        ...
        if outcome.prediction_id in output:
            raise VolatilityAssemblyError("prediction has duplicate question outcomes")
...
def _model_id(prediction: QuestionBoundPrediction) -> str:
    return str(prediction.model_refs[0]).split("@", 1)[0]


def _covers_executable(items: Sequence[Mapping[str, Any]]) -> bool:
    models = {_model_id(row["prediction"]) for row in items if row["prediction"].model_refs}
    return models.issuperset(EXECUTABLE_VOLATILITY_MODEL_IDS)
...
def _select_cutoff(
    ...
    *,
    ...
    if cutoff_at_ns is not None:
        selected = grouped.get(int(cutoff_at_ns), [])
        if not _covers_executable(selected):
            raise VolatilityAssemblyError("requested Volatility cutoff lacks the three executable claims")
        return int(cutoff_at_ns)
    eligible = [cutoff for cutoff, items in grouped.items() if _covers_executable(items)]
    if not eligible:
        raise VolatilityAssemblyError("no Volatility cutoff has three executable journaled claims")
    return max(eligible)


def _unique_executable(selected: Sequence[Mapping[str, Any]]) -> List[Mapping[str, Any]]:
    by_model: Dict[str, Mapping[str, Any]] = {}
    for row in selected:
        model_id = _model_id(row["prediction"])
        if model_id not in EXECUTABLE_VOLATILITY_MODEL_IDS:
            continue
        prior = by_model.get(model_id)
        if prior is None:
            by_model[model_id] = row
            continue
        prior_hash = str(prior["claim"]["integrity"]["content_hash"])
        new_hash = str(row["claim"]["integrity"]["content_hash"])
        if new_hash < prior_hash:
            by_model[model_id] = row
    if set(by_model) != set(EXECUTABLE_VOLATILITY_MODEL_IDS):
        raise VolatilityAssemblyError("Volatility assembly requires the three distinct executable experts")
    output = list(by_model.values())
    output.sort(key=lambda item: (str(item["claim"]["expert_ref"]), str(item["claim"]["integrity"]["content_hash"])))
    return output


def assemble_volatility_question(
    root: Path,
    *,
    known_at_ns: int,
    cutoff_at_ns: Optional[int] = None,
) -> Mapping[str, Any]:
    """Assemble executable Volatility claims from durable journals and competence."""
    root = Path(root).resolve()
    known_at = int(known_at_ns)
    if known_at < 0:
        raise VolatilityAssemblyError("known_at_ns must be non-negative")
    runtime = IntelligenceRuntime(root)
    chain_errors = validate_event_chain(runtime.events())
    if chain_errors:
        raise VolatilityAssemblyError("expert intelligence journal invalid: " + "; ".join(chain_errors))
    state = runtime.state()
    ...
    if isinstance(persisted, Mapping) and persisted.get("known_at_ns") is not None and int(persisted["known_at_ns"]) > known_at:
        raise VolatilityAssemblyError("future competence cannot enter earlier assembly")
    contracts = _volatility_contracts()
    contract_by_model = {}
    ...
    for prediction, journaled_at in _predictions(root):
        if journaled_at > known_at:
            continue
        if prediction.question_ref != VOLATILITY_QUESTION_REF:
            continue
        ...
        if not isinstance(recorded_claim, Mapping):
            raise VolatilityAssemblyError("Volatility claim is not in expert runtime: %s" % claim_hash)
        if recorded_claim.get("integrity", {}).get("content_hash") != claim_hash:
            raise VolatilityAssemblyError("recorded Volatility claim hash mismatch")
        outcome = outcomes.get(prediction.prediction_id)
        scored = any(
            str(item.get("claim_hash")) == claim_hash
            for item in (state.get("scores") or [])
            if isinstance(item, Mapping) and int(item.get("resolved_at_ns") or -1) <= known_at
        )
        rows.append(
            {
    ...
    selected = _unique_executable([row for row in rows if int(row["prediction"].cutoff_at_ns) == cutoff])
    for row in selected:
        if str(row["prediction"].subject_id) != SUBJECT_ID:
            raise VolatilityAssemblyError("wrong subject cannot contribute")
        if row["prediction"].question_ref != VOLATILITY_QUESTION_REF:
            raise VolatilityAssemblyError("wrong-question claims cannot contribute")
        if not row["scored"]:
            raise VolatilityAssemblyError("unresolved or unscored claims cannot contribute")
        outcome = row["outcome"]
        if outcome is None or outcome.status != "RESOLVED":
            raise VolatilityAssemblyError("unresolved or unscored claims cannot contribute")
        if int(outcome.decided_at_ns) > known_at:
            raise VolatilityAssemblyError("future outcomes cannot enter earlier assembly")
    ...
    envelope = {
        "schema_version": "1.0",
        "assembly_id": "QASM-%s" % canonical_hash({"assembly": assembly, "cutoff": cutoff, "known_at": known_at, "family": "VOLATILITY"})[:32],
...
def assemble_and_record_volatility_question(
    root: Path,
    *,
    known_at_ns: int,
    cutoff_at_ns: Optional[int] = None,
) -> Mapping[str, Any]:
    envelope = assemble_volatility_question(root, known_at_ns=known_at_ns, cutoff_at_ns=cutoff_at_ns)
    runtime = IntelligenceRuntime(root)
...
def volatility_assembly_projection(root: Path) -> Mapping[str, Any]:
    root = Path(root).resolve()
    runtime = IntelligenceRuntime(root)
    assemblies = [
        item
        for item in (runtime.state().get("assemblies") or [])
        if isinstance(item, Mapping) and item.get("question_ref") == VOLATILITY_QUESTION_REF
    ]
    ...
    return {
        ...
        if latest is None
            ...
            "source_evidence_groups": latest.get("source_evidence_groups"),
            "source_evidence_refs": latest.get("source_evidence_refs"),
