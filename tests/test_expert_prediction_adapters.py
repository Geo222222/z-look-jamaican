...
class ExpertPredictionAdapterTests(unittest.TestCase):
    ...
    def test_existing_baselines_are_bound_only_to_roles_they_can_defend(self):
        ...
        magnitude_ids = {item["expert_id"] for item in contracts if "MAGNITUDE" in item["expert_id"]}
        self.assertIn("BOOK_CONTEXT_BPS_MAGNITUDE_EXPERT", magnitude_ids)
        self.assertIn("MARKET_WIDE_DRIFT_BPS_MAGNITUDE_EXPERT", magnitude_ids)
