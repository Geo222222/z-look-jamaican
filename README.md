...
`observe → research → hypothesize → design → build → test → simulate → deploy safely → observe live behavior → measure → reflect → keep / improve / suspend / replace → repeat`
...
Only strategies that graduate through the evidence gates in `docs/ZERO_TO_REVENUE.md` may become candidates for production capital.
