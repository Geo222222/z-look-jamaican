...
def build_graph_node(
    *,
    ...
    body: Dict[str, Any] = {
        ...
        "input_refs": [dict(item) for item in refs],
        "method": dict(method),
        "quality": dict(quality),
        "payload": dict(payload),
        "authority": dict(PERCEPTION_GRAPH_AUTHORITY),
    }
    value = dict(body)
...
def validate_graph_node(
    ...
    if not str(node.get("node_id", "")) or not str(node.get("subject_id", "")):
        raise PerceptionGraphError("graph node identity missing")
    cutoff = node.get("cutoff_at_ns")
    known = node.get("known_at_ns")
    if not isinstance(cutoff, int) or not isinstance(known, int) or cutoff < 0 or known < 0 or known > cutoff:
        raise PerceptionGraphError("graph node timing invalid")
    ...
    for ref in refs:
        ...
        parent_layer = str(parent["layer"])
        if GRAPH_LAYER_RANK[parent_layer] >= GRAPH_LAYER_RANK[layer]:
